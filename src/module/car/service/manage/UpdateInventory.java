...
public class UpdateInventory {
	...
	public void deleteInventoryperAppointment(int appointmentno) {
		...
		try {
			String query = "select s.serviceid,c.car_type,g.centerid," + "r.repair_id"
					...
					+ " and req.licenseplateid=c.licenseplateid" + " and g.licenseplateid = c.licenseplateid"
					+ " and a.appointmentno=?";
			...
			if (rs.next()) {
				...
				for (int i = 0; i < basicServiceArray.length; i++) {
					...
					stmt.setString(2,
							rs.getString(2).split(",")[0].trim() + " " + rs.getString(2).split(",")[1].trim());
					...
					for (Map.Entry m : partMap.entrySet()) {
						stmt = connection.prepareStatement(
								"Update inventory set CURRENT_QUANTITY = CURRENT_QUANTITY-? where PARTID=? and CENTERID=? and CAR_MODEL=?");
			...
			// Maintenance
			String query1 = "select s.serviceid,c.car_type,g.centerid," + "m.service_Type"
