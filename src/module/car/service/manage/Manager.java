...
public class Manager {
	...
	private void payroll() {
		...
		try {
			...
			if (emp_start_year.equalsIgnoreCase(current_year)) {
				...
				months_worked = months_left + Integer.parseInt(current_month)
						+ (Integer.parseInt(current_year) - Integer.parseInt(emp_start_year) - 1) * 12;
			}
			...
			for (int i = 0; i < months_worked; i++) {
				...
				if (emp_role.equalsIgnoreCase("1") || emp_role.equalsIgnoreCase("2")) {
					...
					first_month_salary = Integer
							.toString(Integer.parseInt(first_month_days) * Integer.parseInt(salary) / 30);
					...
					if (emp_start_year.equalsIgnoreCase(current_year) || i <= months_left) {
						...
						year_salary = Integer.toString(Integer.parseInt(salary) * pay_period_month);
					...
					if (emp_start_month.equals(current_month)) {
						if (Integer.parseInt(current_day) > 15)
							...
							System.out.println("Employee has not completed a single shift");
					}
					...
					if (emp_start_year.equalsIgnoreCase(current_year)) {
						...
						if (Integer.parseInt(current_day) > 15)
							shifts_worked = shifts_left + ((Integer.parseInt(current_month)) * 2) - 1
									+ ((Integer.parseInt(current_year) - Integer.parseInt(emp_start_year)) - 1) * 24;
						else
							shifts_worked = shifts_left + ((Integer.parseInt(current_month)) * 2) - 2
									+ ((Integer.parseInt(current_year) - Integer.parseInt(emp_start_year)) - 1) * 24;
	...
	private void inventory(String userId) {
		String centerID = "", partID = "", partName = "", currentQuantity = "", thresholdMin = "", minOrder = "",
				carCompany = "";
		int price = 0;
		...
		try {
			...
			stmt = connection.prepareStatement(
					"SELECT partid, part_name, current_quantity, threshold_min, min_order, car_model from Inventory WHERE centerid = ?");
			...
			for (int i = 0; i < partid.size(); i++) {
				stmt = connection
						.prepareStatement("SELECT price from parts_mapping WHERE partid = ? and car_company= ?");
	...
	private void orderHistory() {
		String order_date = "", part_name = "", date = "", supplier_name = "", purchaser_name = "", status = "",
				centerID = "", car_make = "", sender_name = "", rx_centername = "";
		...
		try {
			...
			stmt = connection
					.prepareStatement("SELECT orderid, order_date, part_id, quantity, status, car_make from Orderpart");
			rs = stmt.executeQuery();
			while (rs.next()) {
				orderID = rs.getInt(1);
			...
			for (int i = 0; i < partidlist.size(); i++) {
				try {
					...
					stmt = connection.prepareStatement(
							"SELECT part_name from Inventory where partid=? AND centerID = ? AND car_model = ?");
			...
			/*
			 * for(int i=0; i< orderidlist.size(); i++) { try{ connection=
			 * DBUtility.connectDB(SetupConnection.username, SetupConnection.password);
			 * 
			 * stmt = connection.
			 * prepareStatement("SELECT center_name from service_center where centerid=? ");
			 * stmt.setString(1, senderid_list.get(i));
			 * 
			 * rs = stmt.executeQuery(); while(rs.next()) { sender_name = rs.getString(1);
			 * if(sender_name.equals(null)) { stmt1 = connection.
			 * prepareStatement("SELECT distname from distributor where  distid=? ");
			 * stmt1.setString(1, senderid_list.get(i)); //stmt1.setInt(2,
			 * partidlist.get(i));
			 * 
			 * rs1 = stmt1.executeQuery(); while(rs.next()) { sender_name = rs.getString(1);
			 * sender_name_list.add(sender_name); //System.out.println("Distributor Name: "
			 * + dist_name);
			 * 
			 * } stmt1.close(); rs1.close(); } else sender_name_list.add(sender_name);
			 ...
			 * }catch(SQLException e){
			 * System.out.println("Connection Failed! Check output console");
			 * e.printStackTrace(); DBUtility.close(connection);
			 * 
			 * } }
			 */
			...
			for (int i = 0; i < orderidlist.size(); i++) {
				...
				if (senderid_list.get(i).equals("D0002"))
					...
					System.out.println("Supplier Name: " + senderid_list.get(i));
	...
	private void notifications() {
		...
		try {
			 ...
			 * for(String item : sender_name_list) { System.out.println(item); } }
			 */
			...
			for (int i = 0; i < notificationid_list.size(); i++) {
				...
				if (sender_id_list.get(i).equals("D0002"))
					...
					System.out.println("Supplier Name: " + sender_id_list.get(i));
				System.out.println("Expected Delivery Date: " + expected_date_list.get(i));

				int delayed = Integer.parseInt(actual_date_list.get(i).split("-")[0])
						- Integer.parseInt(expected_date_list.get(i).split("-")[0]);
	...
	private void getOrderDetails(int orderID) {
		...
		String status = "", car_make = "", order_date = "", center_id = "", sx_center_id = "", sx_dist_id = "",
				part_name = "";
		String sender_id = "";
		try {
			connection = DBUtility.connectDB(SetupConnection.username, SetupConnection.password);
			...
			stmt = connection.prepareStatement(
					"SELECT centerid, order_date, part_id, quantity, status, car_make from Orderpart where orderid = ?");
			...
			while (rs.next()) {
				...
				if (sender_id.equals("D0002"))
					...
					System.out.println("Supplier Name: " + sender_id);
		...
		case 1:
			for (int i = 0; i < a_parts.length; i++) {
				try {
					...
					stmt = connection.prepareStatement(
							"SELECT basicserviceid from partrequired_car_model where part_required=? AND TRIM(car_model) = 'Toyota Prius' ");
			...
			for (int i = 0; i < b_parts.length; i++) {
				try {
					...
					stmt = connection.prepareStatement(
							"SELECT basicserviceid from partrequired_car_model where car_model =? AND part_required=?");
...
//		 }
			...
			/*
			 * for(int i=0; i<c_parts_basicservice.length; i++) { try{ connection=
			 * DBUtility.connectDB(SetupConnection.username, SetupConnection.password);
			 * 
			 * stmt = connection.
			 * prepareStatement("INSERT INTO partrequired_car_model VALUES(?,?,?,?)");
			 * stmt.setInt(1, c_parts_basicservice[i]); stmt.setString(2, car_name);
			 * if(i<a_parts.length) { stmt.setInt(3, a_parts[i]); stmt.setInt(4,
			 * a_part_quantity[i]); }
			 * 
			 * else if (i>a_parts.length && i<b_parts.length) { stmt.setInt(3,
			 * b_parts[i-a_parts.length]); stmt.setInt(4,
			 * b_part_quantity[i-a_parts.length]);
			 * 
			 * }
			 * 
			 * else { stmt.setInt(3, c_parts[i-(a_parts.length+b_parts.length)]);
	...
	private void getCarServiceDetails() {
		...
		try {
			...
			stmt = connection.prepareStatement(
					"SELECT TRIM(car_model), TRIM(service_type), TRIM(miles), TRIM(basic_services_used) from basic_service_mapping");
			rs = stmt.executeQuery();
			while (rs.next()) {
	...
	private void getServiceHistory() {
		try {
			...
			String query = "Select S.ServiceID, C.LicensePlateID, C.Type_Recent_Service, R.Mechanic_Preference, A.start_slot, A.App_Date, A.end_slot, Cu.Cust_Name"
					...
					+ " WHERE G.LicensePlateID = C.LicensePlateID" + " AND   C.LicensePlateID = R.LicensePlateID"
					+ " AND   R.Appointmentno = A.Appointmentno" + " AND   A.Appointmentno = L.Appointmentno"
					+ " AND   L.ServiceID = S.ServiceID" + " AND   G.CustID = Cu.CustID";
